local fs = os.getenv("HOME") .. "/.config/yazi/plugins/sudo.yazi/assets/sudo-yazi.sh"
...
local function sudo_link(value)
    ...
    extend_list(args, { ya.quote(fs), "ln" })
...
local function sudo_hardlink(value)
    ...
    extend_list(args, { ya.quote(fs), "hardlink" })
    extend_iter(args, list_map(value.yanked, ya.quote))
...
local function sudo_bulk_rename(value)
    ...
    local prepare_output, prepare_err = Command(fs):arg(prepare_args):output()
...
local function sudo_remove(value)
    ...
    extend_list(args, { ya.quote(fs), "rm" })
