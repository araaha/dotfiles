...
import uuid
from io import \
    open  # ToDo: For open utf8, remove when Python 2 support is skipped

from .errors import *
from .requirements_check import (TexTextRequirementsChecker, defaults,
                                 set_logging_levels)
from .utility import (Cache, ChangeToTemporaryDirectory, CycleBufferHandler,
                      MyLogger, NestedLoggingGuard, Settings, exec_command)
...
# todo: check destination is writeable
...
file_log_channel = logging.handlers.RotatingFileHandler(
    LOG_FILENAME,
...
user_log_channel = CycleBufferHandler(
    capacity=1024)  # store up to 1024 messages
...
class TexText(inkex.EffectExtension):
    ...
    def __init__(self):
        ...
        if previous_exit_code is None:
            ...
            logging.disable(logging.NOTSET)
            logger.debug(
                "Enforcing DEBUG mode due to previous exit code `%d`" %
                previous_exit_code)
        else:
        ...
        self.requirements_checker = TexTextRequirementsChecker(
            logger, self.config)

        if previous_exit_code == EXIT_CODE_OK and "requirements_checker" in self.cache.values:
            self.requirements_checker.inkscape_executable = self.cache[
                "requirements_checker"]["inkscape_executable"]
            self.requirements_checker.available_tex_to_pdf_converters = self.cache[
                "requirements_checker"]["available_tex_to_pdf_converters"]
            self.requirements_checker.available_pdf_to_svg_converters = self.cache[
                "requirements_checker"]["available_pdf_to_svg_converters"]
        else:
            if self.requirements_checker.check() == False:
                ...
                self.cache["requirements_checker"] = {
                    "inkscape_executable":
                    self.requirements_checker.inkscape_executable,
                    "available_tex_to_pdf_converters":
                    self.requirements_checker.available_tex_to_pdf_converters,
                    "available_pdf_to_svg_converters":
                    self.requirements_checker.available_pdf_to_svg_converters,
                }

        super(TexText, self).__init__()

        self.arg_parser.add_argument("--text", type=str, default=None)

        self.arg_parser.add_argument("--preamble-file",
                                     type=str,
                                     default=self.config.get(
                                         'preamble', "default_packages.tex"))

        self.arg_parser.add_argument("--scale-factor",
                                     type=float,
        ...
        self.arg_parser.add_argument("--tex_command",
                                     type=str,
    ...
    def effect(self):
        ...
        with logger.debug("TexText.effect"):
            ...
            preferred_tex_cmd = self.config.get("previous_tex_command",
            ...
            if old_svg_ele is not None:
                ...
                if old_svg_ele.get_meta("version", '<=0.7') == '<=0.7':
                    logger.debug(
                        "Adjust scale factor for node created with TexText<=0.7"
                    )
                ...
                if jac_sqrt != 1.0:
                    logger.debug(
                        "Adjust scale factor to account transformations in inkscape"
                    )
                    current_scale *= old_svg_ele.get_jacobian_sqrt() / jac_sqrt

                alignment = old_svg_ele.get_meta("alignment",
                                                 TexText.DEFAULT_ALIGNMENT)

                current_tex_command = old_svg_ele.get_meta(
                    "texconverter", current_tex_command)
            ...
            if self.options.text is None:
                ...
                if not preamble_file:
                    logger.debug("Using default preamble file `%s`" %
                    ...
                    if not os.path.exists(preamble_file):
                        logger.debug(
                            "Preamble file is NOT found by absolute path")
                        preamble_file = os.path.join(
                            os.path.dirname(self.options.preamble_file),
                            os.path.basename(preamble_file))
                        if not os.path.exists(preamble_file):
                            logger.debug(
                                "Preamble file is NOT found along with default preamble file"
                            )
                            preamble_file = self.options.preamble_file
                        else:
                            logger.debug(
                                "Preamble file is found along with default preamble file"
                            )
                    else:
                ...
                asker = AskTextDefault(
                    ...
                    current_texcmd=current_tex_command,
                    tex_commands=sorted(
                        list(self.requirements_checker.
                             available_tex_to_pdf_converters.keys())),
                    gui_config=gui_config)

                def save_callback(_text,
                                  _preamble,
                                  _scale,
                                  alignment=TexText.DEFAULT_ALIGNMENT,
                                  tex_cmd=TexText.DEFAULT_TEXCMD):
                    return self.do_convert(_text,
                                           _preamble,
                                           _scale,
                                           old_svg_ele,
                                           alignment,
                ...
                def preview_callback(_text, _preamble, _preview_callback,
                                     _tex_command, _white_bg):
                    return self.preview_convert(_text, _preamble,
                                                _preview_callback,
                                                _tex_command, _white_bg)
    ...
    def preview_convert(self, text, preamble_file, image_setter, tex_command,
        ...
        tex_executable = self.requirements_checker.available_tex_to_pdf_converters[
            tex_command]
        ...
        with logger.debug("TexText.do_convert"):
            ...
            try:
                inkscape_version = self.document.getroot().get(
                    'inkscape:version')
                tt_node.set_meta("inkscapeversion",
                                 inkscape_version.split(' ')[0])
            ...
            if old_svg_ele is None:
                with logger.debug("Adding new node to document"):
                    ...
                    tt_node.transform = (
                        Transform(translate=view_center)
                        *  # place at view center
                        # scale
                        Transform(scale=user_scale_factor) *
                        # place node at origin
                        Transform(translate=-node_center) *
                        tt_node.transform  # use original node transform
                    )

                    tt_node.set_meta('jacobian_sqrt',
                                     str(tt_node.get_jacobian_sqrt()))
                ...
                with logger.debug("Replacing node in document"):
                    ...
                    tt_node.align_to_node(old_svg_ele, alignment,
                                          relative_scale)
...
class TexToPdfConverter:
    ...
    def tex_to_pdf(self, tex_command, latex_text, preamble_file):
        ...
        with logger.debug("Converting .tex to .pdf"):
            ...
            try:
                exec_command([tex_command, self.tmp('tex')] +
                             self.LATEX_OPTIONS)
            except TexTextCommandFailed as error:
                if os.path.exists(self.tmp('log')):
                    parsed_log = self.parse_pdf_log()
                    raise TexTextConversionError(parsed_log, error.return_code,
                                                 error.stdout, error.stderr)
                else:
                    raise TexTextConversionError(str(error), error.return_code,
                                                 error.stdout, error.stderr)

            if not os.path.exists(self.tmp('pdf')):
                raise TexTextConversionError("%s didn't produce output %s" %
                                             (tex_command, self.tmp('pdf')))

    def pdf_to_svg(self):
        """Convert the PDF file to a SVG file"""
        exec_command([
            self.checker.inkscape_executable, "--pdf-poppler", "--pdf-page=1",
            "--export-type=svg", "--export-text-to-path",
            "--export-area-drawing", "--export-filename",
            self.tmp('svg'),
            self.tmp('pdf')
        ])
    ...
    def stroke_to_path(self):
        ...
        try:
            exec_command([
                self.checker.inkscape_executable, "-g", "--batch-process",
    ...
    def pdf_to_png(self, white_bg):
        ...
        cmd = [
            self.checker.inkscape_executable, "--pdf-poppler", "--pdf-page=1",
            "--export-type=png", "--export-area-drawing", "--export-dpi=300",
            "--export-filename",
            self.tmp('png'),
            self.tmp('pdf')
        ]
...
class TexTextElement(inkex.Group):
    ...
    def _svg_to_textext_node(self, svg_filename, doc_unit_to_mm):
        from inkex import Defs, ShapeElement, SvgDocumentElement
        ...
        shape_elements = [
            el for el in root if isinstance(el, (ShapeElement, Defs))
        ]
        root.append(self)
    ...
    def _expand_defs(root):
        ...
        from inkex import ShapeElement, Transform
        for el in root:
    ...
    def align_to_node(self, ref_node, alignment, relative_scale):
        ...
        if ref_node.get_meta("pdfconverter", "pstoedit") == "pstoedit":
            # vertical reflection
            revert_flip = Transform(matrix=((1, 0, 0), (0, -1, 0)))
    ...
    def has_colorized_attribute(self):
        ...
        for it_node in self.iter():
            for attrib in ["stroke", "fill"]:
                if attrib in it_node.attrib and it_node.attrib[attrib].lower(
                ).replace(" ", "") not in [
    ...
    def import_group_color_style(self, src_svg_ele):
        ...
        if len(style):
            ...
            color_style_dict = {
                key: value
                for key, value in style.items() if key.lower() in [
                    "fill", "stroke", "opacity", "stroke-opacity",
                    "fill-opacity"
