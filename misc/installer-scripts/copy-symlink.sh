...
mkdir -p "$HOME/.local/share/themes" && mkdir -p "$HOME/.local/share/icons"

ln -sr "$dots/scripts/" "$HOME/"
ln -srf "$dots/.config/*" "$HOME/.config/"
ln -sr "$dots/.local/share/!(themes|icons)"  "$HOME/.local/share/"
...
#binaries
sudo cp -v "$dots/misc/binaries/!(nsxiv|st-0.8.5)" /usr/bin/
